...
public class SphereMovementController : MonoBehaviour
{
   ...
   public GameObject aperture; // Reference to the aperture GameObject
    public Vector3 apertureSize = new Vector3(2f, 2f, 2f); // Size of the cubic aperture
   ...
   void CreateSpheres()
   {
       ...
       for (int i = 0; i < numSpheres; i++)
       {
           ...
           newSphere.transform.localScale = Vector3.one * dotScale; // Set scale of the sphere
            // Set the sphere's position to a known visible position
           newSphere.transform.position = GetRandomPositionInAperture(); // Use the method to get a random position

            Rigidbody rb = newSphere.AddComponent<Rigidbody>();
            rb.isKinematic = true; // Set to kinematic if you want to control movement manually
           ...
           spheres.Add(newSphere); // Add the new sphere to the list
            ...
            SphereCollider sphereCollider = newSphere.GetComponent<SphereCollider>();
            sphereCollider.isTrigger = true; // Set as trigger to detect collisions without physics response
       }
   }
    Vector3 GetRandomPositionInAperture()
    {
        // Generate random x, y coordinates within the aperture size
        float randomX = Random.Range(-apertureSize.x / 2, apertureSize.x / 2);
        float randomY = Random.Range(-apertureSize.y / 2, apertureSize.y / 2);
        
        // Set Z to a fixed value to ensure it stays within the aperture depth
        float fixedZ = 0; // Adjust this value based on your aperture's depth

        // Calculate the position relative to the camera
        Vector3 randomPosition = mainCamera.transform.position + mainCamera.transform.forward * distanceFromCamera
            + new Vector3(randomX, randomY, fixedZ);

        return randomPosition; // Return the random position
    }

    bool IsWithinAperture(Vector3 position)
    {
        // Check if the position is within the aperture boundaries
        return position.x >= (mainCamera.transform.position.x - apertureSize.x / 2) &&
               position.x <= (mainCamera.transform.position.x + apertureSize.x / 2) &&
               position.y >= (mainCamera.transform.position.y - apertureSize.y / 2) &&
               position.y <= (mainCamera.transform.position.y + apertureSize.y / 2) &&
               position.z >= (mainCamera.transform.position.z) && // Assuming the aperture is at the camera's Z position
               position.z <= (mainCamera.transform.position.z + distanceFromCamera);
    }
   ...
   IEnumerator StartMovementLoop()
   {
       while (currentLoop < trialDefs.Count)
       {
           ...
           {
               dot.transform.position = GetRandomPositionInAperture();
   ...
   IEnumerator MoveSignalDots(List<GameObject> signalDots, int direction)
   {
       ...
       while (elapsedTime < visibleTime)
       {
           foreach (var dot in signalDots)
           {
               ...
               // Check if the dot is within the aperture
                if (!IsWithinAperture(dot.transform.position))
                {
                dot.SetActive(false); // Deactivate the dot if it exceeds the aperture
                }
           }
